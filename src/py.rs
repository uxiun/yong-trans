...
pub fn get_shuangpin_tone(c: char, filepath: &str) -> Option<ShuangPinTone> {
  ...
  if let Some(c) = c.to_pinyin() {Some(
    ShuangPinTone::from(
  &PinyinWithToneNumEnd(
        c.with_tone_num_end().to_owned()
      ),
...
impl PinyinWithToneNumEnd {
  ...
  fn split_cv(&self)-> (String, String) {
    ...
    let (sheng, tone) = 
      ...
      // )
      ;
    let mui = &sheng.chars().position(|c|
      "aeuioü".contains(c));
...
impl ShuangPinTone {
  pub fn to_string(&self)->String {
    let s = self.zi.to_string() + self.mu.to_string().as_str()
				+ match self.tone {
...
impl ShuangPinTables {
  fn new()-> Self {
    ShuangPinTables { c: HashMap::new(), v: HashMap::new() }
  }
}
fn shuangpin_table_parser(filepath: &str)
-> ShuangPinTables
{
  let lines = read_lines(filepath)
    .expect("existing filepath");

use nom::{
  IResult,
  character::{is_alphabetic, complete::{multispace1, multispace0, anychar, char as onechar, alpha1, one_of}, is_space}, multi::{separated_list1, many_m_n, fold_many1},
  bytes::complete::{take_while1, tag, is_not}, sequence::{separated_pair, terminated, delimited, preceded, tuple}, error::{ParseError, Error}, InputTakeAtPosition, InputLength, AsChar, Parser, Err,
};
use std::{str::{self, Chars}, collections::HashMap};
...
pub fn read_line_alpha_entry(line: String) -> Option<AlphaEntry> {
  if let Ok((value, (key, space)))
    = tuple((alpha_char, spaces))(line.as_bytes()) {
    Some(
      AlphaEntry {key, value: String::from_utf8(value.to_vec())
...
pub fn read_line_yong_def(line: String) ->  Option<YongDef> {
  ...
  if let Ok(t) = t {Some(
    YongDef {
      spell: String::from_utf8(t.1.0)
        .expect("valid utf8 bytes")
      ,words: t.1.2.into_iter().map(|b|
        String::from_utf8(b.to_vec()).expect("valid utf8 bytes"))
        .collect()
    }
  )
  } else {
...
pub fn read_line_cangjie_def(line: String) ->  Option<YongDef> {
  ...
  if let Ok(t) = t {Some(
    YongDef {
      spell: String::from_utf8(t.1.0.to_vec())
        .expect("valid utf8 bytes")
      ,words: t.1.2.into_iter().map(|b|
        String::from_utf8(b.to_vec()).expect("valid utf8 bytes"))
        .collect()
...
pub fn read_line_custom_add(line: String) -> Option<StringStringsEntry> {
  if let Ok ((rem, result)) = tuple((
    ...
    let mut spells: Vec<String> = result.2.into_iter().map(|e| {
      ...
      .collect();
    let hanzi = String::from_utf8(result.0.to_vec())
      .unwrap();
    // if spells.len() > 1 &&
    if hanzi.chars().count() == 1 {
      spells.pop();
      Some((
        String::from_utf8(result.0.to_vec()).unwrap(),
        spells
      ))
    } else {
      None
...
fn is_vowel_char(c: char)-> bool {"aeuio".contains(c)}
fn is_consonant_char(c: char)-> bool {c.is_ascii_alphabetic() && !is_vowel_char(c)}
  ...
  match js.len() {
    l => {
      ...
      if is_consonant_char(c) {
        ...
        , l => {
          if js[1].as_char() == 'z' {            
            Err(nom::Err::Error(Error { input: i, code: nom::error::ErrorKind::IsNot }))
          } else {
  ...
  if is_consonant_char(first) {
      ...
      anychar::<_, ()>(rem) 
    {
      if second == 'z' {
        ...
        Ok((
          rem, first.to_string().as_bytes().to_vec() 
        ))
      }
    } else {
      ...
      Ok((
        rem
      , first.to_string().as_bytes().to_vec()
      ))
    }
  } else {
    Err(nom::Err::Error(
...
fn trim<'a, I: 'a, F: 'a, O, E: ParseError<&'a I>>(inner: F) -> impl FnMut(&'a I) -> IResult<&'a I, O, E>
  where
...
fn trim_end<'a, I: 'a, F: 'a, O, E: ParseError<&'a I>>(inner: F) -> impl FnMut(&'a I) -> IResult<&'a I, O, E>
  where
  F: Fn(&'a I) -> IResult<&'a I, O, E>,
  &'a I: InputLength + InputTakeAtPosition
  ,<&'a I as InputTakeAtPosition>::Item: AsChar + Clone
...
fn trim_start<'a, I: 'a, F: 'a, O, E: ParseError<&'a I>>(inner: F) -> impl FnMut(&'a I) -> IResult<&'a I, O, E>
  where
  F: Fn(&'a I) -> IResult<&'a I, O, E>,
  &'a I: InputLength + InputTakeAtPosition
  ,<&'a I as InputTakeAtPosition>::Item: AsChar + Clone
